...
*/
/* #define USE_TLS_DH_anon_WITH_AES_256_CBC_SHA */
/* #define USE_TLS_DH_anon_WITH_AES_128_CBC_SHA */
/* #define USE_SSL_DH_anon_WITH_3DES_EDE_CBC_SHA */
/* #define USE_SSL_DH_anon_WITH_RC4_128_MD5 */
...
/******************************************************************************/
/*
 *	Enable the Application Layer Protocol Negotiation extension.  Servers and
 *	Clients will still have to use the required public API to set protocols
 *	and register application callbacks to negotiate the after-SSL protocol.
 *	See the ALPN section in the developer's guide for information
 */
/* #define USE_ALPN */
...
/*
	...
	This is a non-standard implementation of ZLIB compression.  The only
	data that is compressed and decompressed internally by the library
	is the FINISHED message of an initial handshake.

	Re-handshaking is not supported.

	On the application data sending side:
	After a successful handshake with USE_ZLIB_COMPRESSION enabled, the user
	should call matrixSslIsSessionCompressionOn to test whether that mode
	has been successfully negotiated.  If PS_TRUE, the user must manually zlib
	deflate any application data before calling the Matrix encryption
	functions. Do not compress more plaintext data in a single record than the
	maximum allowed record size to remain compatible with 3rd party SSL
	implementations.

	On the application data receiving side:
	Applications must test for MATRIXSSL_APP_DATA_COMPRESSED as the return
	code from matrixSslReceivedData.  If found, the data must be zlib inflated
